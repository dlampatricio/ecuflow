...
export default function ChatPage() {
  ...
  return (
    <main className="flex-1 pt-20 pb-20 flex flex-col max-w-3xl mx-auto w-full px-4">
      <div className="flex items-center gap-3 py-6 border-b border-white/30 dark:border-white/10">
      ...
      {items.length > 0 && (
        <div className="my-6 p-4 rounded-2xl bg-white/40 dark:bg-slate-900/50 backdrop-blur-xl border border-white/60 dark:border-white/10">
          ...
          <div className="flex gap-3 overflow-x-auto pb-2">
            {items.map((item) => {
              ...
              return (
                <div
                  key={item.product.id}
                  className="shrink-0 flex items-center gap-2 bg-white/40 dark:bg-slate-900/50 rounded-xl p-2"
                >
      ...
      <div className="flex-1 overflow-y-auto py-4 space-y-4">
          ...
          >
            <div
              className={cn(
                ...
                msg.role === 'user'
                  ? 'bg-cyan-500'
                  : 'bg-white/40 dark:bg-slate-900/50 backdrop-blur-xl border border-white/60 dark:border-white/10',
              )}
            >
              {msg.role === 'user' ? (
                user?.imageUrl ? (
            ...
            <div
              className={cn(
                ...
                msg.role === 'user'
                  ...
                  : 'rounded-tl-md text-slate-700 dark:text-white bg-white/60 dark:bg-slate-900/40 backdrop-blur-2xl shadow-[0_10px_40px_-10px_rgba(0,0,0,0.1)] dark:shadow-[0_20px_50px_-12px_rgba(0,0,0,0.5)] border border-white/40 dark:border-white/10',
              )}
            >
        ...
        {isTyping && (
          <div className="flex gap-3 animate-fade-up">
            <div className="w-10 h-10 rounded-2xl bg-white/40 dark:bg-slate-900/50 backdrop-blur-xl border border-white/60 dark:border-white/10 flex items-center justify-center">
              <Bot className="h-5 w-5 text-slate-700 dark:text-white" />
            </div>
            <div className="bg-white/40 dark:bg-slate-900/50 backdrop-blur-xl border border-white/60 dark:border-white/10 rounded-2xl rounded-tl-md px-5 py-3">
      ...
      <form
        ...
        className="py-4 border-t border-white/30 dark:border-white/10"
      >
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 h-14 rounded-2xl px-5 text-sm placeholder:text-slate-400 dark:placeholder:text-white/40 text-slate-700 dark:text-white focus:outline-none focus:ring-2 focus:ring-cyan-500/30 focus:border-cyan-500/50 transition-all bg-white/60 dark:bg-slate-900/40 backdrop-blur-2xl shadow-[0_10px_40px_-10px_rgba(0,0,0,0.1)] dark:shadow-[0_20px_50px_-12px_rgba(0,0,0,0.5)] border border-white/40 dark:border-white/10"
          />
          <button
            type="submit"
