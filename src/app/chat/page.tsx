...
import { MessageCircle, Send, User, Bot, ShoppingBag } from "lucide-react";
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: "1",
      role: "assistant",
      content: "¡Hola! 👋 Bienvenido a Ecuflow. ¿En qué puedo ayudarte hoy? Puedes preguntarme sobre productos, precios o coordinar el envío de tu pedido.",
      timestamp: new Date(),
    },
  ]);
    ...
    <div className="flex min-h-screen flex-col">
      ...
      <main className="flex-1 pt-20 pb-20 flex flex-col max-w-3xl mx-auto w-full px-4">
        <div className="flex items-center gap-3 py-6 border-b border-white/30 dark:border-white/[0.1]">
          ...
          <div>
            <h1 className="font-bold text-lg text-slate-800 dark:text-white">Ecuflow</h1>
            <p className="text-xs text-slate-500 dark:text-white/50">Estamos aquí para ayudarte</p>
          </div>
        </div>
              ...
              {items.map((item) => {
                ...
                >
                  ...
                  <div className="text-sm">
                    <p className="font-medium text-slate-700 dark:text-white line-clamp-1">{item.product.name}</p>
            ...
            >
              <div
                className={cn(
                  ...
                  msg.role === "user" ? "bg-cyan-500" : "bg-white/40 dark:bg-slate-900/50 backdrop-blur-xl border border-white/60 dark:border-white/[0.1]"
                )}
              >
                {msg.role === "user" ? (
            ...
            <div className="flex gap-3 animate-fade-up">
              ...
              <div className="bg-white/40 dark:bg-slate-900/50 backdrop-blur-xl border border-white/60 dark:border-white/[0.1] rounded-2xl rounded-tl-md px-5 py-3">
                <div className="flex gap-1">
                  <span className="w-2 h-2 rounded-full bg-slate-400 dark:bg-white/40 animate-bounce" style={{ animationDelay: "0ms" }} />
                  <span className="w-2 h-2 rounded-full bg-slate-400 dark:bg-white/40 animate-bounce" style={{ animationDelay: "150ms" }} />
                  <span className="w-2 h-2 rounded-full bg-slate-400 dark:bg-white/40 animate-bounce" style={{ animationDelay: "300ms" }} />
                </div>
              </div>
            </div>
          )}
